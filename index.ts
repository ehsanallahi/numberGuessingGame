...
async function main() {
  ...
  let actualNumber: number = Math.floor(Math.random() * 10 + 1);
  ...
  while (play) {
    ...
    const playAgainAnswer: { playAgain: boolean } = await inquirer.prompt([
      {
        name: "playAgain",
    ...
    if (playAgainAnswer.playAgain) {
      ...
      console.log(chalk.blue(figlet.textSync("Thanks for playing!", { horizontalLayout: 'full' })));
      play = false;
    }
  }
}
